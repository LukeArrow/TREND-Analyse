...
export class TrendAnalysisCard extends LitElement {
    ...
    private renderChanges(): Template {
        ...
        const deltaPercentValue = data.deltaPercent !== undefined && Object.is(data.deltaPercent, -0) ? 0 : data.deltaPercent;
        ...
        return html`
            <div class="net-change ${data.trend}">
                ...
                <div class="value-container">
                    ...
                    ` : nothing}
                    ${showPercentage ? html`
                        <span class="percentage ${data.trend}">${deltaPercentValue !== undefined ? `${deltaPercentValue > 0 ? '+' : ''}${deltaPercentValue.toFixed(1)}` : '0.0'}%</span>
                    ` : nothing}
                </div>
            </div>
        `;
